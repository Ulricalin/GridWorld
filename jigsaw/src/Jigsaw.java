...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		while (!openList.isEmpty()) {
			...
			if (currentJNode.equals(endJNode)) {
				...
				Vector<JigsawNode> tempVec = findFollowJNodes(currentJNode);
				for (JigsawNode node : tempVec) {
					openList.add(node);
				}
			}
	...
	/**（Demo+实验二）启发式搜索。访问节点数大于25000个则认为搜索失败。
	...
	private void estimateValue(JigsawNode jNode) {
		...
		jNode.setEstimatedValue(s  + distance);
	}
	
	private int getDistance(JigsawNode jNode) {
		int distance = 0;
		int dimension = JigsawNode.getDimension();
		int finalState[] = endJNode.getNodesState();
		int jNodeState[] = jNode.getNodesState();
		for (int index = 1 ; index<dimension*dimension; index++) {
			int r1 = (index-1)/dimension;
			int c1 = (index-1)%dimension;
			for (int i = 1; i<dimension*dimension; i++) {
				if (jNodeState[index] == finalState[i]) {
					if (index == i) break;
					int r2 = (i-1)/dimension;
					int c2 = (i-1)%dimension;
